...
// Erstellt: 2026-06-19
// Quelle: C:/Users/Vannon/Desktop/SyxBridge_Live/core/src/text-core.js
...
function applyTranslations(content, replacements, translations, plugin) {
  ...
  for (const r of sorted) {
    ...
    if (translated !== undefined) {
      ...
      // ZERO-WIDTH WATERMARK: Unsichtbarer Unicode-Marker (​ / ‌)
    // Wird nach dem ersten Wort injiziert (split-basiert).
    // Zufallsauswahl zwischen ZWSP und ZWNJ pro String.
    ...
    if (typeof translated === 'string' && translated.length > 0) {
      ...
      const marker = WATERMARK_CONFIG.ZW_MARKERS[Math.floor(Math.random() * WATERMARK_CONFIG.ZW_MARKERS.length)];
