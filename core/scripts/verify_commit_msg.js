#!/usr/bin/env node
/**
 * verify_commit_msg.js — RULE 3 Härtung (Never-Ending Story Edition)
 * 
 ...
 * 
 * RULE 2 (Commit-Tagebuch / Never-Ending Story):
 *   - 500-1000 Wörter für normale Commits (Erzählung/Tagebuch-Stil)
 *   - 50 Wörter Minimum für triviale Commits (nur Doc oder ≤1 Datei + <10 Zeilen)
 *   - Cross-Referenz: Letzte 5 Commits müssen aufeinander verweisen
 *   - Sidejokes: Gemini ODER Meta, nie beide im gleichen Commit
 *   - Keine Bulletpoints, keine Change-Logs — nur Prosa
 * 
...
if (wordCount < RULE2_MIN_WORDS) {
    ...
    console.error('');
    if (isTrivial) {
        console.error('Trivial commit detected (doc-only or <10 lines).');
        console.error('Minimum: 50 words. A precise sentence will do.');
    } else {
        console.error('RULE 2 _Commit-Tagebuch / Never-Ending Story:');
        console.error('KEIN Commit ohne 500-1000 Wörter Prosa.');
        console.error('Schreibe als Tagebuch-Eintrag aus der Ich-Perspektive.');
        console.error('Die Commits eines Tages bilden eine fortlaufende');
        console.error('Erzählung — nie einzeln, immer als Kapitel.');
        console.error('Cross-Referenz auf die letzten 5 Commits ist Pflicht.');
        console.error('Sidejokes: Gemini ODER Meta — nie beide im gleichen Commit.');
        console.error('Keine Bulletpoints. Keine Change-Logs. Nur Prosa.');
    }
    console.error('');
    process.exit(1);
}
...
console.log('');    console.log(`  ${stagedFiles.length} staged file(s) — all referenced`);
    console.log(`  RULE 2 word count: ${wordCount} words (≥${RULE2_MIN_WORDS})`);
    if (isTrivial) {
        console.log(`  Type: TRIVIAL (doc-only or <10 lines)`);
    } else {
        console.log(`  Format: Never-Ending Story / Commit-Tagebuch`);
    }
    console.log('');
    console.log('Fertig... Claude wird eh meckern');
    console.log('');
    process.exit(0);
